...
<title>スキルブック：<?php print($bangou);?></title>
...
if($movies_result = $mysqli->query($movies_query)){
	...
	$count = $count + 1;
		if($count != 5){
	echo '<td>投稿日&nbsp;'.$movies_row['post_time'].'<br /><img src="../'.$movies_row['thumbnail'].'" width="150" height="75"><br /><a href="movie_see.php?ban='.$movies_row['id'].'">'.mb_strimwidth($movies_row['movie_title'], 0, 22, "...","UTF-8").'</a></td>';
		}
		if($count == 5){
	echo '<td>投稿日&nbsp;'.$movies_row['post_time'].'<br /><img src="../'.$movies_row['thumbnail'].'" width="150" height="75"><br /><a href="movie_see.php?ban='.$movies_row['id'].'">'.mb_strimwidth($movies_row['movie_title'], 0, 22, "...","UTF-8").'</a></td><tr>';
		$count = 0;
		}
	}
	$movies_result->free(); 
