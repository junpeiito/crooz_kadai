...
<div class="pd3">
 <p class="lefttxt1">
<?php
	$kaihatsu_row = mysql_query('select * from kaihatsu where user = "'.$_SESSION['user']['id'].'"');
	while ($row = mysql_fetch_assoc($kaihatsu_row)) {
		print("タイトル：");
		...
		print('</a>');
		print("：");
		print('<a href="kaihatsu_delete.php?number=');
		print($row['id']);
		print('">');
		print("削除する。");
		print('</a>');
...
<a href="create.php">新規開発シリーズの作成</a>
